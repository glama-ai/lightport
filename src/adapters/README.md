...
Enables all providers to support OpenAI's newer Responses API.
...
**Native Providers** (bypass adapter): `openai`, `azure-openai`, `x-ai`, `groq`, `openrouter`
...
**Native Providers** (bypass adapter): `anthropic`, `bedrock`, `vertex-ai` (Claude models), `azure-ai` (Claude models)
...
it's a non-standard Lightport extension (via `content_blocks`).
