...
export function transformChatCompletionsToResponses(
  ...
  return {
    ...
    completed_at: overallStatus === 'completed' ? Math.floor(Date.now() / 1000) : null,
    status: overallStatus,
    incomplete_details: overallStatus === 'incomplete' ? { reason: 'max_output_tokens' } : null,
    ...
    parallel_tool_calls: req.parallel_tool_calls ?? RESPONSE_DEFAULTS.parallel_tool_calls,
    text: textFormat,
    top_p: req.top_p ?? RESPONSE_DEFAULTS.top_p,
    presence_penalty: req.presence_penalty ?? RESPONSE_DEFAULTS.presence_penalty,
    frequency_penalty: req.frequency_penalty ?? RESPONSE_DEFAULTS.frequency_penalty,
