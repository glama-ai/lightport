...
export function constructConfigFromRequestHeaders(
  ...
  const azureConfig = {
    ...
    azureManagedClientId: requestHeaders[`x-${POWERED_BY}-azure-managed-client-id`],
  ...
  const azureAiInferenceConfig = {
    ...
    azureExtraParameters: requestHeaders[`x-${POWERED_BY}-azure-extra-parameters`],
    azureAdToken: requestHeaders[`x-${POWERED_BY}-azure-ad-token`],
    azureAuthMode: requestHeaders[`x-${POWERED_BY}-azure-auth-mode`],
    azureManagedClientId: requestHeaders[`x-${POWERED_BY}-azure-managed-client-id`],
    azureEntraClientId: requestHeaders[`x-${POWERED_BY}-azure-entra-client-id`],
  ...
  const awsExtraConfig = {
    ...
    awsServerSideEncryption: requestHeaders[`x-${POWERED_BY}-amz-server-side-encryption`],
  ...
  const sagemakerConfig = {
    ...
    amznSagemakerTargetVariant: requestHeaders[`x-${POWERED_BY}-amzn-sagemaker-target-variant`],
    amznSagemakerTargetContainerHostname:
      requestHeaders[`x-${POWERED_BY}-amzn-sagemaker-target-container-hostname`],
    amznSagemakerInferenceId: requestHeaders[`x-${POWERED_BY}-amzn-sagemaker-inference-id`],
  ...
  if (requestHeaders[`x-${POWERED_BY}-config`]) {
    let parsedConfigJson = parseJson<Record<string, any>>(requestHeaders[`x-${POWERED_BY}-config`]);
