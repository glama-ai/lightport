...
describe('Google Media Resolution Support', () => {
  describe('Top-level media_resolution', () => {
    it('should transform media_resolution to generationConfig', () => {
      ...
      const transformedRequest = transformUsingProviderConfig(GoogleChatCompleteConfig, params);

      expect(transformedRequest.generationConfig).toBeDefined();
      expect(transformedRequest.generationConfig.mediaResolution).toBe('MEDIA_RESOLUTION_HIGH');
    });
    ...
    it('should support all media resolution values', () => {
      ...
      resolutions.forEach((resolution) => {
        ...
        expect(transformedRequest.generationConfig.mediaResolution).toBe(resolution);
      });
    });
    ...
    it('should not add mediaResolution if not provided', () => {
      ...
      expect(transformedRequest.generationConfig?.mediaResolution).toBeUndefined();
    });
  });
  ...
  describe('Inline media_resolution', () => {
    it('should transform inline media_resolution for base64 images', () => {
      ...
      expect(transformedRequest.contents[0].parts[0]).toHaveProperty('inlineData');
    ...
    it('should transform inline media_resolution for file URLs', () => {
      ...
      expect(transformedRequest.contents).toHaveLength(1);
      expect(transformedRequest.contents[0].parts).toHaveLength(1);
      expect(transformedRequest.contents[0].parts[0]).toHaveProperty('fileData');
      expect(transformedRequest.contents[0].parts[0]).toHaveProperty(
        'mediaResolution',
        'MEDIA_RESOLUTION_MEDIUM',
      );
    ...
    it('should not add mediaResolution if not provided inline', () => {
      ...
      expect(transformedRequest.contents[0].parts).toHaveLength(1);
      expect(transformedRequest.contents[0].parts[0]).toHaveProperty('inlineData');
      expect(transformedRequest.contents[0].parts[0]).not.toHaveProperty('mediaResolution');
    });
    ...
    it('should support both top-level and inline media_resolution', () => {
      ...
      expect(transformedRequest.generationConfig.mediaResolution).toBe('MEDIA_RESOLUTION_LOW');
      ...
      expect(transformedRequest.contents[0].parts[1]).toHaveProperty('fileData');
      ...
      expect(transformedRequest.contents[0].parts[2]).toHaveProperty('fileData');
      expect(transformedRequest.contents[0].parts[2]).not.toHaveProperty('mediaResolution');
    });
  });
