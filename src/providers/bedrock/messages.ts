...
import { AnthropicMessagesStreamResponse } from '../../middlewares/lightport/types';
import {
  ContentBlockParam,
  DocumentBlockParam,
  ImageBlockParam,
...
import { AnthropicMessageDeltaEvent, AnthropicMessageStartEvent } from '../anthropic-base/types';
...
const transformAnthropicBeta = (params: BedrockMessagesParams, providerOptions?: Options) => {
  ...
  return anthropicBeta.filter((beta: string) => beta !== 'prompt-caching-scope-2026-01-05');
};
...
export const BedrockMessagesResponseTransform = (
  ...
  if ('output' in response) {
    ...
    const responseObj: MessagesResponse = {
      // TODO: shorten this
      id: 'lightport-' + crypto.randomUUID(),
...
function createContentBlockStartEvent(
  ...
  const contentBlockStartEvent: RawContentBlockStartEvent = parseJson(
    ANTHROPIC_CONTENT_BLOCK_START_EVENT,
  );
...
export const BedrockConverseMessagesStreamChunkTransform = (
  ...
  const parsedChunk: BedrockChatCompleteStreamChunk = parseJson(responseChunk);
  ...
  if (
    ...
    if (streamState.currentContentBlockIndex !== -1) {
      const previousBlockStopEvent: RawContentBlockStopEvent = parseJson(
        ANTHROPIC_CONTENT_BLOCK_STOP_EVENT,
      );
  ...
  if (parsedChunk.usage) {
    const messageDeltaEvent: AnthropicMessageDeltaEvent = parseJson(ANTHROPIC_MESSAGE_DELTA_EVENT);
    ...
    const contentBlockStopEvent: RawContentBlockStopEvent = parseJson(
      ANTHROPIC_CONTENT_BLOCK_STOP_EVENT,
    );
...
function getMessageStartEvent(fallbackId: string, gatewayRequest: Params) {
  const messageStartEvent: AnthropicMessageStartEvent = parseJson(ANTHROPIC_MESSAGE_START_EVENT);
...
export const BedrockConverseAnthropicMessagesConfig: ProviderConfig = getMessagesConfig({
...
export const BedrockAnthropicMessagesStreamChunkTransform = (responseChunk: string) => {
  const parsedChunk: AnthropicMessagesStreamResponse = parseJson(responseChunk);
