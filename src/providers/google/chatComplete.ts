...
import { GOOGLE_GENERATE_CONTENT_FINISH_REASON, LightportGeminiParams } from './types';

const transformGenerationConfig = (params: LightportGeminiParams) => {
  ...
  if (params?.response_format?.type === 'json_schema') {
    ...
    generationConfig['responseJsonSchema'] =
      params?.response_format?.json_schema?.schema ?? params?.response_format?.json_schema;
  }
  if (params?.thinking) {
...
export const GoogleChatCompleteConfig: ProviderConfig = {
          ...
          if (message.role === 'assistant' && message.tool_calls) {
            message.tool_calls.forEach((tool_call: ToolCall) => {
              ...
              parts.push({
                functionCall: {
                  ...
                  args: parseJson(tool_call.function.arguments),
                },
                ...(thought_signature && {
            ...
            message.content.forEach((c: ContentType) => {
              if (c.type === 'text') {
                ...
                const { url, mime_type: passedMimeType, media_resolution } = c.image_url || {};
                if (!url) return;
  ...
  tool_choice: {
    ...
    default: (params: Params) => {
      ...
      if (googleMapsTool) {
        toolConfig.retrievalConfig = googleMapsTool.function?.parameters?.retrievalConfig;
        return toolConfig;
      }
      return;
    },
...
export const GoogleChatCompleteStreamChunkTransform: (
  ...
  const parsedChunk: GoogleGenerateContentResponse = parseJson(chunk);
