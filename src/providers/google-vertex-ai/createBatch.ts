...
export const GoogleBatchCreateConfig: ProviderConfig = {
  model: {
    ...
    transform: (params: Record<string, any>) => {
      ...
      if (params.model?.startsWith('projects') || params.model?.startsWith('publishers')) {
        return params.model;
      }
...
export const GoogleBatchCreateResponseTransform = (response: Response, responseStatus: number) => {
