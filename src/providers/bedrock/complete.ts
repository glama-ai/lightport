...
export const BedrockLlamaCompleteStreamChunkTransform: (
  ...
  chunk = chunk.trim();
  const parsedChunk: BedrockLlamaStreamChunk = parseJson(chunk);
...
export const BedrockTitanCompleteStreamChunkTransform: (
  ...
  let chunk = responseChunk.trim();
  chunk = chunk.trim();
  const parsedChunk: BedrockTitanStreamChunk = parseJson(chunk);
...
export const BedrockAnthropicCompleteStreamChunkTransform: (
  ...
  const parsedChunk: BedrockAnthropicStreamChunk = parseJson(chunk);
  if (parsedChunk.stop_reason) {
    return [
      `data: ${JSON.stringify({
        id: fallbackId,
...
export const BedrockMistralCompleteStreamChunkTransform: (
  ...
  let chunk = responseChunk.trim();
  chunk = chunk.trim();
  const parsedChunk: BedrocMistralStreamChunk = parseJson(chunk);
