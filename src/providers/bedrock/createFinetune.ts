...
export const BedrockCreateFinetuneConfig: ProviderConfig = {
  ...
  hyperparameters: {
    ...
    transform: (value: FinetuneRequest) => {
      ...
      const learningRateMultiplier = value.hyperparameters?.learning_rate_multiplier;
      const batchSize = value.hyperparameters?.batch_size;
      return {
        epochCount: epochCount ? String(epochCount) : undefined,
        learningRateMultiplier: learningRateMultiplier ? String(learningRateMultiplier) : undefined,
  ...
  output_file: {
    ...
    default: (value: FinetuneRequest) => {
      ...
      const uri = trainingFile.substring(0, trainingFile.lastIndexOf('/') + 1) + value.suffix;
      return {
        s3Uri: uri,
      };
    },
  ...
  job_name: {
    ...
    default: (value: FinetuneRequest & { job_name: string }) => {
      return value.job_name ?? `lightport-finetune-${crypto.randomUUID()}`;
    },
  },
  role_arn: {
    param: 'roleArn',
...
export const BedrockCreateFinetuneResponseTransform: (
  ...
  if (!OK_STATUS.includes(responseStatus) || 'error' in response) {
    return (
      BedrockErrorResponseTransform(response as unknown as BedrockErrorResponse) ||
