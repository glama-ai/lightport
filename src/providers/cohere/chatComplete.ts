...
export const CohereChatCompleteStreamChunkTransform: (
  ...
  strictOpenAiCompliance: boolean,
  gatewayRequest: Params,
) => string = (responseChunk, fallbackId, streamState, strictOpenAiCompliance, gatewayRequest) => {
  ...
  let parsedChunk: CohereChatCompletionStreamChunk;
  try {
    parsedChunk = parseJson(chunk);
  } catch (error) {
    captureException({
      error,
      extra: { chunk, responseChunk },
      message: 'failed to parse Cohere stream chunk',
    });
    return '';
  }
      ...
      choices: [
        {
          ...
          delta: {
            role: 'assistant',
            content:
              (parsedChunk as V2ChatStreamResponse.ContentDelta).delta?.message?.content?.text ??
              '',
            tool_calls: (parsedChunk as V2ChatStreamResponse.ToolCallDelta).delta?.message
              ?.tool_calls,
          },
          logprobs: null,
          finish_reason: null,
