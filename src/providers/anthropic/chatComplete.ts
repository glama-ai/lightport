...
const transformAssistantMessage = (msg: Message): AnthropicMessage => {
  ...
  let inputContent: ContentType[] | string | undefined = msg.content_blocks ?? msg.content;
 ...
 */
const buildAnthropicOutputConfig = (params: Params): Record<string, any> | undefined => {
      ...
      transform: (params: Params) => {
        ...
        if (params.messages) {
          params.messages.forEach((msg: Message & PromptCache) => {
            ...
            } else if (SYSTEM_MESSAGE_ROLES.includes(msg.role) && typeof msg.content === 'string') {
              systemMessages.push({
                ...(msg?.cache_control && {
                  cache_control: { type: 'ephemeral' },
                }),
  ...
  const AnthropicChatCompleteStreamChunkTransform: (
    ...
    if (parsedChunk.type === 'message_delta' && parsedChunk.usage) {
      const {
        ...
        output_tokens = 0,
        cache_creation_input_tokens = streamState.usage?.cache_creation_input_tokens ?? 0,
        cache_read_input_tokens = streamState.usage?.cache_read_input_tokens ?? 0,
      } = parsedChunk.usage;
      const promptTokens =
        input_tokens + (cache_read_input_tokens ?? 0) + (cache_creation_input_tokens ?? 0);
      const totalTokens = promptTokens + output_tokens;
      const shouldSendCacheUsage = cache_read_input_tokens || cache_creation_input_tokens;
      return (
        `data: ${JSON.stringify({
          id: fallbackId,
          object: 'chat.completion.chunk',
    ...
    if (isToolBlockStart && parsedChunk.content_block) {
      ...
      const contentBlockIndex = parsedChunk.index ?? streamState.toolIndex;
    ...
    } else if (isToolBlockDelta && parsedChunk.index !== undefined) {
