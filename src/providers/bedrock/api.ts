...
const getMethod = (fn: endpointStrings, transformedRequestUrl: string, c: GatewayContext) => {
  if (fn === 'proxy') {
    return c.req.method;
  }
  if (fn === 'uploadFile') {
...
const isAnthropicModelOnMessagesRoute = (provider: string, fn: endpointStrings) => {
  return provider === ANTHROPIC && (fn === 'messages' || fn === 'stream-messages');
};
...
const BedrockAPIConfig: BedrockAPIConfigInterface = {
  getBaseURL: async ({ providerOptions, fn, gatewayRequestURL, params }) => {
    if (providerOptions.awsAuthType === 'serviceRole') {
      providerOptions.awsRegion = providerOptions.awsRegion || getRegionFromEnv();
    }
