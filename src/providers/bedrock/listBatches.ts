...
export const BedrockListBatchesResponseTransform = (
  ...
  if ('invocationJobSummaries' in response) {
    const batches = response.invocationJobSummaries.map((batch) => ({
      ...
      status: batch.status,
      input_file_id: encodeURIComponent(batch.inputDataConfig.s3InputDataConfig.s3Uri),
      output_file_id: encodeURIComponent(batch.outputDataConfig.s3OutputDataConfig.s3Uri),
