import { BatchEndpoints, GOOGLE_VERTEX_AI, fileExtensionMimeTypeMap } from '../../globals';
...
export const getAccessToken = async (
  ...
  try {
    ...
    captureException({ error, message: 'failed to get Google Vertex AI access token' });
    return '';
  }
};
...
export async function getGcpIdentityAccessToken(env?: Record<string, any>) {
  try {
    if (Environment({}).GCP_AUTH_MODE === 'workload') {
      ...
      const resp = await requestCache(env).get<{ access_token: string }>(cacheKey, {
        useLocalCache: true,
      });
    ...
    captureException({ error, message: 'failed to get Google Vertex AI auth token' });
    return undefined;
  }
}
...
export async function getModelProviderFromModelID(
  ...
  const projectId = options.vertexProjectId || options.vertexServiceAccountJson?.project_id;
  ...
  try {
    ...
    if (finetuneModelId) {
      ...
      const finetuneModelData = (await finetuneModel.json()) as Record<string, unknown>;
      const finetuneModelName =
        finetuneModelData.baseModel ?? (finetuneModelData.source_model as any)?.baseModel;
    ...
    logger.error({ err: error, model, options }, 'unable to fetch base model from endpoint');
    return null;
  }
}
...
export const derefer = (
  ...
  if ('$ref' in node && typeof node.$ref === 'string') {
    ...
    if (defKey && target) {
      ...
      Object.assign(node, resolved, siblings);
    }
  }
...
export const getThoughtSignature = (model?: string, thoughtSignature?: string) => {
