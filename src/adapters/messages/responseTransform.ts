...
export function transformChatCompletionsToMessages(
  ...
  for (const choice of response.choices || []) {
    ...
    stopReason = STOP_REASON_MAP[choice.finish_reason] || ANTHROPIC_STOP_REASON.end_turn;
    ...
    if (msg.tool_calls?.length) {
      for (const tc of msg.tool_calls) {
        if (tc.type === 'function') {
          ...
          try {
            const parsed = parseJson<Record<string, any>>(tc.function.arguments || '{}');
            input = parsed !== null && typeof parsed === 'object' ? parsed : {};
          } catch (err) {
            logger.warn(
              { err, provider, toolName: tc.function?.name },
