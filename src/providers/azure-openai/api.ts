...
const AzureOpenAIAPIConfig: ProviderAPIConfig = {
  ...
  headers: async ({ providerOptions, fn }) => {
    const { apiKey, azureAdToken, azureAuthMode, azureEntraScope } = providerOptions;
    const nodeFetchHeader = fn === 'uploadFile' ? { 'use-node-fetch': true } : {};
    if (azureAdToken) {
      return {
    ...
    if (azureAuthMode === 'entra') {
      const { azureEntraTenantId, azureEntraClientId, azureEntraClientSecret } = providerOptions;
  ...
  getProxyEndpoint: ({ reqPath, reqQuery, providerOptions }) => {
    ...
    const isAzureAgents = providerOptions['azureEntraScope'] === 'https://ai.azure.com/.default';
