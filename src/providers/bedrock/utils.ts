...
async function getAssumedWebIdentityCredentials(
  ...
  if (!envVars.AWS_WEB_IDENTITY_TOKEN_FILE || !envVars.AWS_ROLE_ARN) {
    return null;
  }

  try {
    const effectiveRegion = awsRegion || getRegionFromEnv(env) || 'us-east-1';
    const roleArn = envVars.AWS_ROLE_ARN;
    const cacheKey = `assumed-web-identity-${envVars.AWS_WEB_IDENTITY_TOKEN_FILE}-role-${roleArn}-region-${effectiveRegion}`;
    ...
    const token = await fs.readFile(envVars.AWS_WEB_IDENTITY_TOKEN_FILE, 'utf8');

    const credentials = await fetchWebIdentityCredentials(token, roleArn, effectiveRegion);
    ...
    logger.info({ err: error }, 'error from getAssumedWebIdentityCredentials');
  }
  return null;
}
...
async function getPodIdentityCredentials(
  ...
  try {
    ...
    const credentialFullUri = envVars.AWS_CONTAINER_CREDENTIALS_FULL_URI;
    const cacheKey = `assumed-pod-identity-${envVars.AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE}-region-${effectiveRegion}`;
    ...
    const token = await fs.readFile(envVars.AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE, 'utf8');

    const credentials = await fetchPodIdentityCredentials(token.trim(), credentialFullUri);
    ...
    logger.info({ err: error }, 'failed to get Pod Identity credentials');
  }
  return null;
}
...
async function getCredentialsFromECSContainer(
  ...
  const relativeUri = Environment(env).AWS_CONTAINER_CREDENTIALS_RELATIVE_URI;
  if (!relativeUri) {
    return null;
  }
  ...
  const credentials = await fetchECSContainerCredentials(relativeUri);
  ...
  const derivedRegion =
    getRegionFromEnv(env) || (await fetchECSRegionFromMetadata(env)) || 'us-east-1';
...
async function getSTSAssumedCredentials(
  ...
  const accessKeyId: string = accessKey || Environment(env).AWS_ASSUME_ROLE_ACCESS_KEY_ID || '';
  const secretAccessKey: string =
    secretKey || Environment(env).AWS_ASSUME_ROLE_SECRET_ACCESS_KEY || '';
  const region =
    awsRegion || Environment(env).AWS_ASSUME_ROLE_REGION || getRegionFromEnv() || 'us-east-1';
...
async function getSourceRoleCredentials(
  ...
  const sourceRoleExternalId = Environment(env).AWS_ASSUME_ROLE_SOURCE_EXTERNAL_ID || '';
...
export async function getAssumedRoleCredentials(
  ...
  if (!accessKeyId && !secretAccessKey) {
    try {
      ...
      logger.error({ err: error }, 'error while assuming source role');
      return null;
    }
  }
  ...
  secretAccessKey = secretAccessKey || Environment(env).AWS_ASSUME_ROLE_SECRET_ACCESS_KEY;

  if (!accessKeyId && !secretAccessKey) {
    let credentials: AWSCredentials | undefined | null = getCredentialsFromEnvironment();
    ...
    if (!credentials) {
      try {
        ...
        logger.error({ err: error }, 'failed to get AWS credentials');
      }
    }
    if (!credentials) {
      try {
...
export const transformAnthropicAdditionalModelRequestFields = (
  ...
  // response_format → output_config.format, reasoning_effort → output_config.effort
  {
    ...
    if (params.response_format?.type === 'json_schema') {
      outputConfig.format = {
        type: 'json_schema',
        schema: params.response_format.json_schema?.schema ?? params.response_format.json_schema,
      };
    }
...
export const getFoundationModelFromInferenceProfile = async (
  ...
  try {
    ...
    const cachedFoundationModel = await requestCache(env).get<string>(cacheKey, {
      useLocalCache: true,
    });
    ...
    logger.debug({ err: error }, 'error mapping inference profile to foundation model');
  }
};
...
export function getS3EncryptionHeaders(env: Record<string, any>) {
  ...
  return {
    ...
    ...(kmsEncryptionCustomerKeyMD5 && {
      'x-amz-server-side-encryption-customer-key-MD5': kmsEncryptionCustomerKeyMD5,
    }),
  };
}
