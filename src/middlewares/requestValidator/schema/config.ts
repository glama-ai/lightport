...
const TRUSTED_CUSTOM_HOSTS = (_c?: GatewayContext) => {
  const envVar = Environment({}).TRUSTED_CUSTOM_HOSTS;
  if (!envVar) {
  ...
  .object({
    strategy: z
      .object({
        mode: z
          .string()
          .refine((value) => ['single', 'loadbalance', 'fallback', 'conditional'].includes(value), {
            message:
        ...
        conditions: z
          .array(
            z.object({
              query: z.object({}),
              // oxlint-disable-next-line unicorn/no-thenable -- legitimate data property name for conditional strategy
              then: z.string(),
            }),
          )
          .optional(),
              ...
              .array(
                z
                  ...
                  .refine(
                    ...
                    {
                      message: 'hash_fields must start with "metadata.", "params.", or "headers."',
                    },
                  ),
              )
              .optional(),
    ...
    cache: z
      .object({
        mode: z.string().refine((value) => ['simple', 'semantic'].includes(value), {
          message: "Invalid 'cache.mode' value. Must be one of: simple, semantic",
        }),
...
export function isValidCustomHost(customHost: string, c?: GatewayContext) {
  try {
    ...
    if (value.indexOf('api.lightport') > -1) return false;
    if (DISALLOWED_SCHEMES.some((scheme) => value.startsWith(scheme))) return false;
    ...
    if (BLOCKED_TLDS.some((tld) => host.endsWith(tld) && host !== 'localhost')) {
      return false;
    }
...
function isPrivateIPv4(ip: string): boolean {
  ...
  return IPV4_RANGES.PRIVATE.some((range) => ipInt >= range.start && ipInt <= range.end);
}

function isReservedIPv4(ip: string): boolean {
  const ipInt = ipv4ToInt(ip);
  return IPV4_RANGES.RESERVED.some((range) => ipInt >= range.start && ipInt <= range.end);
}
...
function isAlternativeIPRepresentation(host: string, parts: string[]): boolean {
  ...
  if (
    parts.length >= 2 &&
    parts.length < 4 &&
    parts.every((p) => VALIDATION_PATTERNS.DIGITS_ONLY.test(p) && Number(p) <= 255) &&
  ...
  if (parts.length >= 2 && parts.length < 4) {
    if (parts.every((p) => VALIDATION_PATTERNS.DIGITS_ONLY.test(p) && Number(p) <= 255)) {
