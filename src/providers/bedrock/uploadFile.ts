...
const isHeader = (chunk: string) => {
  return chunk.startsWith('Content-Disposition: form-data') || chunk.includes('Content-Type:');
};
...
class AwsMultipartUploadHandler {
  ...
  constructor(
    ...
    this.url = new URL(`https://${bucket}.s3.${region}.${awsEndpointDomain}/${objectKey}?uploads`);
  ...
  async initiateMultipartUpload() {
    ...
    if (!uploadIdMatch) throw new Error('Failed to parse UploadId from response');
    this.uploadId = uploadIdMatch[1];
    return uploadIdMatch[1];
  }

  async pushCurrentPart(passThrough: PassThrough, partNumber: number, lastPart?: boolean) {
    ...
    const transform = new Transform({
      transform: async function (chunk, encoding, callback) {
        ...
        try {
          ...
          if ((passThrough.writableLength || passThrough.readableLength) >= MIN_CHUNK_SIZE) {
            const partLength = await classThis.pushCurrentPart(passThrough, partNumber);
            partNumber++;
          ...
          if (result.purpose === 'fine-tune') {
            ...
            const modelInput = transformUsingProviderConfig(providerConfig, json.body);
            const transformedLine = {
          ...
          result.error = new Error(`Found invalid JSON at line ${line}: ${error}`);
        }
      ...
      flush: async (callback) => {
        const partLength = await classThis.pushCurrentPart(passThrough, partNumber, true);
        classThis.contentLength += partLength;
        callback();
      },
    });
...
export const BedrockUploadFileRequestHandler: RequestHandler<ReadableStream> = async ({
  providerOptions,
  requestBody,
  requestHeaders,
  c,
  ...
  try {
    ...
    const { awsRegion, awsS3Bucket, awsBedrockModel: modelParam } = providerOptions;

    const awsS3ObjectKey = providerOptions.awsS3ObjectKey || crypto.randomUUID() + '.jsonl';
    ...
    if (!awsS3Bucket || !modelParam) {
      return new Response(
        JSON.stringify({
          status: 'failure',
          message:
            'Please make sure you have x-lightport-aws-s3-bucket, x-lightport-aws-s3-object-key and x-lightport-aws-bedrock-model headers provided.',
        }),
        {
          status: 400,
          headers: {
    ...
    if (awsBedrockModel.includes('arn:aws')) {
      const foundationModel = awsBedrockModel.includes('foundation-model/')
        ...
        : await getFoundationModelFromInferenceProfile(awsBedrockModel, providerOptions, cEnv);
      if (foundationModel) {
    ...
    if (!awsBedrockModel || !awsS3Bucket)
      throw new Error('awsBedrockModel and awsS3Bucket are required to upload a file');
    ...
    if (!purpose || error) {
      await onError?.();
      const _errorMessage = !purpose ? 'Purpose is invalid' : error?.message || 'Unknown error';
      return new Response(
        JSON.stringify({
          error: {
    ...
    return parseProviderErrorResponse({
      captureMessage: 'failed to parse Bedrock upload file error response',
      error,
      provider: BEDROCK,
    });
  }
