...
function createComposedRequestTransform(
  ...
  return (responsesRequest: Params, _requestHeaders: Record<string, string>): Params => {
    const chatCompletionsRequest = transformResponsesToChatCompletions(responsesRequest as any);
...
function createComposedResponseTransform(
  ...
  return (providerResponse: any, responseStatus: number) => {
    const chatCompletionsResponse = providerChatCompleteResponseTransform(providerResponse);
    if ('error' in chatCompletionsResponse) {
      return chatCompletionsResponse;
    }
    return transformChatCompletionsToResponses(chatCompletionsResponse, responseStatus, provider);
  };
}
...
describe('Responses Adapter', () => {
  describe('transformResponsesToChatCompletions', () => {
    test('should transform simple string input to messages', () => {
      ...
      const result = transformResponsesToChatCompletions(responsesRequest as any);
  ...
  describe('transformChatCompletionsToResponses', () => {
    ...
    test('should transform reasoning/thinking content to summary array format', () => {
      ...
      expect((reasoningOutput as any).summary[0].text).toBe('Let me calculate 2+2. That equals 4.');
    ...
    test('should handle thinking block with text field instead of thinking field', () => {
      ...
      const reasoningOutput = result.output[0];
      expect(reasoningOutput.type).toBe('reasoning');
      expect((reasoningOutput as any).summary[0].text).toBe('Processing the request...');
    });
  });

  describe('createComposedRequestTransform', () => {
    test('should compose Responses → ChatCompletions → Anthropic transforms', () => {
      const composedTransform = createComposedRequestTransform(MockAnthropicChatCompleteConfig);
  ...
  describe('createComposedResponseTransform', () => {
    test('should compose Anthropic → ChatCompletions → Responses transforms', () => {
      const mockAnthropicTransform = vi.fn().mockImplementation((response) => ({
        id: 'chatcmpl-123',
        object: 'chat.completion',
        created: 1700000000,
        model: 'claude-3-sonnet-20240229',
      ...
      const result = composedTransform(anthropicResponse, 200) as OpenAIResponse;
