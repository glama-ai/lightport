...
export async function fetchEntraIdToken(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error({ errorMessage }, 'error from Entra');
      return undefined;
    }
    ...
    logger.error({ err: error }, 'Entra ID token generation error');
    return undefined;
  }
}
...
export async function fetchManagedIdentityToken(
  ...
  try {
    ...
    if (!response.ok) {
      const errorMessage = await response.text();
      logger.error({ errorMessage }, 'error from Managed Identity');
      return undefined;
    }
    ...
    logger.error({ err: error }, 'Managed Identity token generation error');
    return undefined;
  }
}
...
export async function fetchAzureWorkloadIdentityToken(
  ...
  try {
    ...
    logger.error({ err: error }, 'Workload Identity token generation error');
  }
  return undefined;
}
