...
/*
  ...
  NOTE: Lightport does not restrict messages to alternate user and assistant roles, this is to support more flexible use cases.
*/
const transformMessagesForLLama3Prompt = (messages: Message[]) => {
  let prompt: string = '';
  prompt += LLAMA_3_SPECIAL_TOKENS.PROMPT_START + '\n';
  messages.forEach((msg) => {
    prompt += LLAMA_3_SPECIAL_TOKENS.ROLE_START + msg.role + LLAMA_3_SPECIAL_TOKENS.ROLE_END + '\n';
...
const BedrockMistralChatCompleteConfig: ProviderConfig = {
  messages: {
    ...
    transform: (params: Params) => {
      let prompt: string = '';
      if (params.messages) prompt = transformMessagesForMistralPrompt(params.messages);
      return prompt;
    },
  },
  max_tokens: {
...
const BedrockTitanChatCompleteConfig: ProviderConfig = {
  messages: {
    param: 'inputText',
    required: true,
    transform: (params: Params) => {
...
export const BedrockUploadFileTransformerConfig: Record<string, ProviderConfig> = {
  ...
  titan: BedrockTitanChatCompleteConfig,
