...
for (const token of NO_PROXY_TOKENS) {
  try {
    ...
    logger.warn({ err: error, token }, 'invalid NO_PROXY pattern');
  }
}
...
function matchNoProxyToken(host: string, port: string | undefined, tokenRaw: string): boolean {
...
const REQUEST_TIMEOUT =
  env?.REQUEST_TIMEOUT && parseInt(env.REQUEST_TIMEOUT) ? parseInt(env.REQUEST_TIMEOUT) : undefined;
...
export function buildAgents(agentConfig: AgentConfig) {
  if (agentConfig.tls) {
    store.httpsAgent = new UndiciAgent({
      allowH2: false,
      ...(REQUEST_TIMEOUT ? { headersTimeout: REQUEST_TIMEOUT, bodyTimeout: REQUEST_TIMEOUT } : {}),
