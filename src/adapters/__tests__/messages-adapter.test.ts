...
import { transformStreamChunk, createStreamState } from '../messages/streamTransform';
...
describe('Messages → Chat Completions Request Transform', () => {
  test('simple message', () => {
    const result = transformMessagesToChatCompletions(fixtures.requests.simple());
    expect(result.messages).toHaveLength(1);
  });

  test('with string system', () => {
    const result = transformMessagesToChatCompletions(fixtures.requests.withSystem());
    expect(result.messages).toHaveLength(2);
    expect(result.messages![0].role).toBe('system');
  });

  test('with array system', () => {
    const result = transformMessagesToChatCompletions(fixtures.requests.withSystemArray());
    expect(result.messages).toHaveLength(2);
    expect(result.messages![0].role).toBe('system');
  ...
  test('transforms tools correctly', () => {
    const result = transformMessagesToChatCompletions(fixtures.requests.withTools());
  ...
  test('transforms tool_use and tool_result correctly', () => {
    const result = transformMessagesToChatCompletions(fixtures.requests.withToolUse());
  ...
  test('transforms multimodal content correctly', () => {
    const result = transformMessagesToChatCompletions(fixtures.requests.multimodal());
  ...
  test('maps all parameters correctly', () => {
    const result = transformMessagesToChatCompletions(fixtures.requests.allParams());
...
describe('Chat Completions → Messages Response Transform', () => {
  ...
  test('passes through errors', () => {
    ...
    const result = transformChatCompletionsToMessages(errorResponse as any, 400, 'openai');
...
describe('Native Provider Detection', () => {
  test.each([
    ...
    expect(supportsMessagesApiNatively(provider, 'claude-3.5-sonnet')).toBe(expected);
  });
});
...
describe('Round-trip Transformation', () => {
  ...
  test('response maintains structure through transform', () => {
    ...
    const messagesResponse = transformChatCompletionsToMessages(chatResponse as any, 200, 'openai');
    ...
    expect((messagesResponse.content[0] as any).text).toBe(chatResponse.choices[0].message.content);
  });
});
