...
const getProjectRoute = (providerOptions: Options, inputModel: string): string => {
  ...
  const { provider } = getModelAndProvider(inputModel);
...
const GoogleApiConfig: ProviderAPIConfig = {
  ...
  headers: async ({ providerOptions, gatewayRequestBody, c: _c }) => {
    const { apiKey, vertexServiceAccountJson, vertexAuthType } = providerOptions;
    let authToken = apiKey;
    if (vertexServiceAccountJson) {
      authToken = await getAccessToken(vertexServiceAccountJson, {});
    } else if (vertexAuthType && vertexAuthType === 'workload') {
      authToken = await getGcpIdentityAccessToken({});
    }

    const anthropicBeta =
      providerOptions?.['anthropicBeta'] ?? (gatewayRequestBody as Params)?.['anthropic_beta'];
  ...
  getEndpoint: ({
    ...
    const projectRoute = getProjectRoute(providerOptions, inputModel);
