...
export const OpenAIChatCompleteJSONToStreamResponseTransform: (
  ...
  const { id, model, system_fingerprint, choices, citations, service_tier } = response;
  ...
  for (const [index, choice] of choices.entries()) {
    ...
    if (
      ...
      const individualWords: Array<string> = [];
      for (let i = 0; i < choice.message.content.length; i += 500) {
        individualWords.push(choice.message.content.slice(i, i + 500));
      }
