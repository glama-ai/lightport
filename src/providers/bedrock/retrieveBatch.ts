...
export const BedrockRetrieveBatchResponseTransform = (
  ...
  if ('jobArn' in response) {
    return {
      ...
      input_file_id: encodeURIComponent(response.inputDataConfig.s3InputDataConfig.s3Uri),
      output_file_id: encodeURIComponent(response.outputDataConfig.s3OutputDataConfig.s3Uri),
