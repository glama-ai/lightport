 ...
 */
import { ANTHROPIC, AZURE_AI_INFERENCE, BEDROCK, GOOGLE_VERTEX_AI } from '../../globals';

// Providers that natively support the Messages API
const PROVIDERS_THAT_HANDLE_MESSAGES_FOR_ALL_MODELS = new Set([ANTHROPIC, BEDROCK]);
 ...
 */
export function supportsMessagesApiNatively(provider: string, model: string): boolean {
  return (
    (PROVIDERS_THAT_HANDLE_MESSAGES_FOR_ANTHROPIC_MODELS.has(provider.toLowerCase()) &&
