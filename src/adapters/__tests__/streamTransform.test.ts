...
import { transformStreamChunk, createStreamState } from '../responses/streamTransform';
...
describe('Stream Transform', () => {
  describe('transformStreamChunk', () => {
    ...
    test('should emit delta events for content chunks', () => {
      ...
      expect(state.hasEmittedTextOutputItem).toBe(true);
    });
    ...
    test('should handle tool call arguments delta', () => {
      ...
      // First, register the tool call with its name
      const nameChunk =
        'data: {"choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_123","type":"function","function":{"name":"get_weather","arguments":""}}]}}]}';
      transformStreamChunk(nameChunk, state);
    ...
    test('should track usage from chunks', () => {
      ...
      const chunk = 'data: {"choices":[],"usage":{"prompt_tokens":10,"completion_tokens":5}}';
  ...
  describe('sequence_number', () => {
    test('should include sequence_number in all events starting from 0', () => {
      const state = createStreamState();
      const chunk =
        'data: {"id":"chat-123","object":"chat.completion.chunk","created":1700000000,"model":"gpt-4","choices":[{"index":0,"delta":{"role":"assistant","content":"Hi"},"finish_reason":null}]}';
      ...
      const events = result!
        ...
        .map((e) => {
          ...
          return parseJson<Record<string, any>>(dataLine!.slice(5));
        });

      // Initial events: created, in_progress, output_item.added, content_part.added, delta
      ...
      expect(events[4].sequence_number).toBe(4); // response.output_text.delta
    });

    test('should increment sequence_number across multiple chunks', () => {
      const state = createStreamState();

      // First chunk (init only, no content)
      const chunk1 =
        'data: {"id":"chat-123","model":"gpt-4","choices":[{"delta":{"role":"assistant"}}]}';
      transformStreamChunk(chunk1, state);

      // After init (created + in_progress), sequence should be at 2
      expect(state.sequenceNumber).toBe(2);

      // Content chunk (first text triggers output_item.added + content_part.added + delta)
      const chunk2 = 'data: {"choices":[{"delta":{"content":"Hello"}}]}';
      transformStreamChunk(chunk2, state);

      // After first content: +3 events (output_item.added, content_part.added, delta)
      expect(state.sequenceNumber).toBe(5);

      // Another content chunk (only delta)
  ...
  describe('accumulated text', () => {
    test('should accumulate text across content deltas', () => {
      ...
      // Content chunks
      transformStreamChunk('data: {"choices":[{"delta":{"content":"Hello"}}]}', state);
      expect(state.accumulatedText).toBe('Hello');

      transformStreamChunk('data: {"choices":[{"delta":{"content":" "}}]}', state);
      expect(state.accumulatedText).toBe('Hello ');

      transformStreamChunk('data: {"choices":[{"delta":{"content":"World"}}]}', state);
    ...
    test('should include accumulated text in response.completed event', () => {
      ...
      expect(completedEvent.response.output[0].content[0].text).toBe('Hello World!');
    });
    ...
    test('should include accumulated text in all done events', () => {
      ...
      expect(result).toContain('"part":{"type":"output_text","text":"Test message"');

      // Check response.output_item.done
      expect(result).toContain('"content":[{"type":"output_text","text":"Test message"');
    });
  });
  ...
  describe('input_tokens_details in stream', () => {
    test('should include input_tokens_details in response.completed usage', () => {
      ...
      expect(completedEvent.response.usage.input_tokens_details.cached_tokens).toBe(0);
      expect(completedEvent.response.usage.output_tokens_details).toBeDefined();
      expect(completedEvent.response.usage.output_tokens_details.reasoning_tokens).toBe(0);
    });
  });
  ...
  describe('Full Stream Sequence', () => {
    ...
    test('should correctly transform a tool call streaming sequence', () => {
      ...
      // Verify completion events for tool calls
      expect(allOutput).toContain('response.function_call_arguments.done');
      expect(allOutput).toContain('response.completed');

      // Verify accumulated arguments in done event
      const doneEvents = allOutput
        .split('\n\n')
        .filter((e) => e.includes('response.function_call_arguments.done'))
        .map((e) => {
          const dataLine = e.split('\n').find((l) => l.startsWith('data:'));
          return parseJson<Record<string, any>>(dataLine!.slice(5));
        });
      ...
      expect(doneEvents[0].name).toBe('get_weather');
      expect(doneEvents[0].arguments).toBe('{"location": "Paris"}');

      // Verify tool call in response.completed output
      const completedEvent = allOutput
        .split('\n\n')
        .filter((e) => e.includes('response.completed'))
        .map((e) => {
          const dataLine = e.split('\n').find((l) => l.startsWith('data:'));
      ...
      const functionCallOutput = completedEvent.response.output.find(
        (o: any) => o.type === 'function_call',
      );
      expect(functionCallOutput).toBeDefined();
      expect(functionCallOutput.name).toBe('get_weather');
      expect(functionCallOutput.call_id).toBe('call_abc123');
